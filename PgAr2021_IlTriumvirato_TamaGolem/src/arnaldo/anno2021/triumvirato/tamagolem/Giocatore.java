...
public class Giocatore {
	...
	public void loadCurrentGolem() {
		System.out.println(String.format(Constants.SECRET_INPUT_MESSAGE, nome,BattleHandler.getOtherPlayerName(this)));
		BattleHandler.pressEnterToContinue();
		...
		for(int i=1;i<=BattleHandler.getP();i++) {
			...
			ArrayList<Pietra> primePerTipo=BattleHandler.getArrayFirstGemsByType();
			ArrayList<String> elencoGemme=BattleHandler.getElencoGemme();
			...
			this.getGolemInPlay().addGem(riferimentoGemma); //copy happens, this object is thus different and will not be removed
			BattleHandler.removeGem(riferimentoGemma); //this object is a pointer and thus the gem will be effectively removed from the global list		
